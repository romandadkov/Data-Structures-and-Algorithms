...
public:
  ...
  void set_lower_bounds(std::vector<int> out_lbs, std::vector<int> in_lbs)
  {
  ...
  int get_edge_flow(size_t id) const
  {
    const auto &edge = get_edge(id);
...
FlowGraph read_data()
{
  ...
  graph.set_lower_bounds(std::move(out), std::move(in));

  return graph;
}

void bfs(const FlowGraph &graph, int from, int to, vector<int> &pred)
{
  queue<int> q;
  q.push(from);
  ...
  while (!q.empty())
  {
    ...
    for (auto id : graph.get_ids(cur))
    {
      ...
      if (pred[e.to] == -1 && e.capacity > e.flow && e.to != from)
      {
        pred[e.to] = id;
        q.push(e.to);
      }
...
int max_flow(FlowGraph &graph, int s, int t)
{
  ...
  do
  {
    ...
    if (pred[t] != -1)
    {
      ...
      {
        min_flow = std::min(min_flow, graph.get_edge_flow(u));
      }
      for (int u = pred[t]; u != -1; u = pred[graph.get_edge(u).from])
      {
